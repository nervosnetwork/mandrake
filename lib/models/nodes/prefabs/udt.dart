...
AstNode _inputCells() {
  ...
  and.addChild(_isDefaultSecpCell(Int64(0)), and.slots.first.id);
  and.addChild(_isSimpleUdtCell(Int64(0), Int64(0)), and.slots.last.id);
...
AstNode _transferCell() {
  ...
  result.addChild(_assembleSecpLock(Int64(2)), result.addSlot('secp lock').id);
  result.addChild(_assembleUdtType(Int64(0)), result.addSlot('udt type').id);
...
AstNode _changeCell() {
  ...
  result.addChild(_assembleSecpLock(Int64(1)), result.addSlot('secp lock').id);
  result.addChild(_assembleUdtType(Int64(0)), result.addSlot('udt type').id);
...
AstNode _isSimpleUdtCell(Int64 argIndex, Int64 paramIndex) {
...
AstNode _assembleSecpLock(Int64 paramIndex) {
  final result = AstNode(valueType: ValueType.script);
  result.addChild(bytesValue(secpTypeHash), result.addSlot('sepc type hash').id);
  result.addChild(uintValue(Int64(1)), result.addSlot('1').id);
  result.addChild(param(paramIndex), result.addSlot('param').id);
  return result;
}

AstNode _assembleUdtType(Int64 paramIndex) {
  final result = AstNode(valueType: ValueType.script);
  result.addChild(bytesValue(udtCodeHash), result.addSlot('udt code hash').id);
  result.addChild(uintValue(Int64(0)), result.addSlot('0').id);
  result.addChild(param(paramIndex), result.addSlot('param').id);
  return result;
}
...
AstNode _assembleSecpCellDep() {
  ...
  final secpCellBytes = bytesValue(secpCellDepDevnet);
