...
List<AstNode> convertUdtGetBalance(PrefabNode node) {
  return [
    autoLayout(_balance(node.properties)..name = 'balance', initialPosition: node.position),
  ];
}

List<AstNode> convertUdtTransfer(PrefabNode node) {
  return [
    autoLayout(_transfer(node.properties)..name = 'transfer', initialPosition: node.position),
  ];
}
...
List<AstNode> convertUdt(PrefabNode node) {
  return [
    autoLayout(
      _ready(node.properties)..name = 'ready',
...
AstNode _ready(List<PrefabProperty> props) {
...
AstNode _balance(List<PrefabProperty> props) {
  ...
  final tokens = mapFuns(_inputCells(props), [
...
AstNode _transfer(List<PrefabProperty> props) {
  ...
  outputs.addChild(_transferCell(props), outputs.addSlot('transfer').id);
  outputs.addChild(_changeCell(props), outputs.addSlot('change').id);
  transaction.addChild(
    outputs,
...
AstNode _typeCells(List<PrefabProperty> props) {
...
AstNode _inputCells(List<PrefabProperty> props) {
  final queryCells = AstNode(valueType: ValueType.queryCells);
  final and = OperationNode(valueType: ValueType.and);
  and.addChild(_isDefaultSecpCell(Int64(0), props), and.slots.first.id);
  and.addChild(
    _isSimpleUdtCell(
      Int64(0),
      Int64(0),
...
AstNode _transferCell(List<PrefabProperty> props) {
  final result = AstNode(valueType: ValueType.cell);
  result.addChild(_transferValue(), result.addSlot('value').id);
  result.addChild(_assembleSecpLock(Int64(2), props), result.addSlot('secp lock').id);
  result.addChild(_assembleUdtType(Int64(0), props), result.addSlot('udt type').id);
  result.addChild(_transferTokens(), result.addSlot('tokens').id);
  return result;
}

AstNode _changeCell(List<PrefabProperty> props) {
  final result = AstNode(valueType: ValueType.cell);
  result.addChild(_changeCapacities(props), result.addSlot('value').id);
  result.addChild(_assembleSecpLock(Int64(1), props), result.addSlot('secp lock').id);
  result.addChild(_assembleUdtType(Int64(0), props), result.addSlot('udt type').id);
  result.addChild(_changeTokens(props), result.addSlot('tokens').id);
  return result;
}
...
AstNode _totalCapacities(List<PrefabProperty> props) {
  ...
  map.addChild(_inputCells(props), map.addSlot('cells').id);
...
AstNode _changeCapacities(List<PrefabProperty> prop) {
  return subtract(_totalCapacities(prop), _transferValue());
}

AstNode _changeTokens(List<PrefabProperty> props) {
  final result = AstNode(valueType: ValueType.slice);
  result.addChild(uintValue(Int64(0)), result.addSlot('0').id);
  result.addChild(uintValue(Int64(16)), result.addSlot('16').id);
  result.addChild(subtract(_balance(props), _transferTokens()), result.addSlot('change token').id);
  return result;
}

AstNode _isDefaultSecpCell(Int64 argIndex, List<PrefabProperty> props) {
...
AstNode _assembleSecpLock(Int64 paramIndex, List<PrefabProperty> props) {
...
AstNode _assembleUdtType(Int64 paramIndex, List<PrefabProperty> props) {
  final result = AstNode(valueType: ValueType.script);
  result.addChild(
...
AstNode _assembleSecpCellDep(List<PrefabProperty> props) {
  final result = AstNode(valueType: ValueType.cellDep);

  final outPoint = AstNode(valueType: ValueType.outPoint);
  final secpCellBytes = bytesValue(findPropValue(props, 'Secp256k1 cell dep') ?? secpCellDepDevnet);
